...
function createMap(bikeStations) { 
  ...
  let myMap = L.map('map-id', {
    ...
    zoom: mapZoomLevel,
    layers:[lightmap,bikeStations] // why directly bikestations??
  });
...
function createMarkers(response) {
  ...
  // Loop through the stations array.
    ...
    for (let i = 0; i < stations.length; i++) {
      let bikeMarker = L.marker([stations[i].lat, stations[i].lon]).bindPopup(`<h3>${stations[i].name}</h3>`);
 ...
 // let bikeMarkersLayer = L.layerGroup(bikeMarkers); // Kasia solution
